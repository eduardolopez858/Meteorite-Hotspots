...
## My agent's setup
Using the chosen dataset, the main features my agent will be utilizing is mass, location, and a timeline of the metoerite landings since it's purpose is to predict the likelyhood of a meteorite landing on a given location. Also, additionally infering it's danger level given it's frequencies and mass. 
...
After the preprocessing method has done it's work and extracted the mass, lat, and lon features of the dataset required for the meteorite hotspot detection, then the frequencies method will perform one of the main computations of the agent, essential for the inference later on in the pipeline. The frequencies method utilizes a modern unsupervised clustering algorithm known as DBSCAN, Density Based Clustering with Applications with Noise. After training and doing a baseline comparison with a centriod based clustering algorithm (k-means), it demonstrated promising results in meteorite hotspot detection and outperformed k-means due to the noisy instances and complex clusters the dataset contained. The algorithm's hyperparameters (eps) and (min_samples) were set to 0.3 and 10, tuned for close to perfect cluster assignment. 
...
From here, the next method in the pipeline is a probablistic one that uses new user input (city location and year range) and the previous frequencies method as evidence. Since DBSCAN can only perform hard assignment on each of the instances towards a cluster and the goal of the agent is to predict the likelihood of a new city and year $instance_i$ witnessing a meteorite landing by soft assigning it to some pre-trained $cluster_i$, then the method will calculate such assignment using the $density_i$ * $weight_i$ of each $cluster_i$ and collecting the highest, most probable assignment. The density of each $cluster_i$ will be calculated using a SVM soft scoring kernel density estimator, specifically the Gaussian RBF kernel ```density = np.exp(-dist / (2 * sig**2))```. This kernel allows for smooth and simple calculations (using euclidean distance, mean, variance, etc.) so that each cluster is assigned a density. Then, using those densities and weights , the highest likelihood that some city $instance_i$ belonging to a $cluster_i$ can be retrieved in a probabilistic way beyond DBSCAN's capabilities. Keeping in mind the new $instance_i$ will be the location and year the user wants to make inference on.
...
Finally, we can have more fun with the agent and infer the danger level of the user's new given location and year range by calculating the average of all the meteorites masses of the soft assigned cluster (hotspot).
